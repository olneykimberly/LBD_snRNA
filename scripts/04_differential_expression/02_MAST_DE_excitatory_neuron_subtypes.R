...
obj <- readRDS(paste0("../rObjects/subclusters/", project_ID, "_neuron_subclustered.rds"))
...
sum(obj$subcluster %in% bad_subclusters)         

# ensures subtype name is in object
bad_present <- intersect(bad_subclusters, unique(obj$subcluster))
cells_keep <- colnames(obj)[!(obj$subcluster %in% bad_present)]
...
pdf_name <- paste0("../results/DEG_MAST_RNA_pct0.25_celltype_subtypes/excitatory_neuron/UMAP_clusters_and_subtype_annotation.pdf")
...
pdf_name <- paste0("../results/DEG_MAST_RNA_pct0.25_celltype_subtypes/excitatory_neuron/UMAP.rpca_clusters_and_subtype_annotation.pdf")
...
count_bar
pdf_name <- paste0("../results/DEG_MAST_RNA_pct0.25_celltype_subtypes/excitatory_neuron/subtype_count_per_group.pdf")
pdf(pdf_name, width = 6, height = 6)
print(count_bar)
dev.off()
...
out_base <- "../results/DEG_MAST_RNA_pct0.25_celltype_subtypes/excitatory_neuron/"
