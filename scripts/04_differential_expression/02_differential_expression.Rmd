...
```{r}
# dataObject.pcg <- subset(x = dataObject, features = protein_coding_genes$gene_name) #pcg - protein coding genes
...
pcg <- protein_coding_genes$gene_name
# remove MT genes
pcg_filtered <- intersect(pcg, rownames(dataObject.PSCT))

#counts <- dataObject@assays$SCT$counts

table(Idents(dataObject.PSCT))
de_interneuron_AD_vs_control <- FindMarkers(dataObject.PSCT, 
                       ident.1 = "interneuron_AD_AT", ident.2 = "interneuron_CONTROL", 
                       ...
                       densify = TRUE, 
                       verbose = TRUE)
de_interneuron_AD_vs_control$gene <- rownames(de_interneuron_AD_vs_control)

write.table(de_interneuron_AD_vs_control, "de_interneuron_AD_vs_control.txt", sep = "\t", quote = FALSE)
```
