...
output_dir <- "../results/volcanoes_pseudobulk/"
```
...
for (cell_type in cell_types) {
  ...
  for (comp in comparisons) {
    ...
    file_name <- paste0(cell_type,"/DEG_pseudobulk_voom_", cell_type, "_", pathology_A, "_vs_", pathology_B, ".tsv")
    file_path <- paste0("../results/DEGs_pseudobulk_limma_voom/", file_name)
    plot_title <- paste0(cell_type, " (", pathology_A, " vs ", pathology_B, ")")
    output_pdf <- paste0("../results/volcanoes_pseudobulk/", cell_type, "_", pathology_A, "_vs_", pathology_B, ".pdf")
    ...
    # 2. Check for the required columns (assuming 'gene' column exists) # logFC AveExpr	t	P.Value	adj.P.Val	B	gene
    if (!all(c("logFC", "P.Value", "adj.P.Val", "gene") %in% colnames(data))) {
      message(paste("Skipping:", file_name, "- Missing required columns (logFC, P.Value, adj.P.Val, or gene)."))
      next
    }
    ...
    # Up-regulated (Red): adj.P.Val < qval AND logFC > posFC
    data$color_adjpval_0.05[data$adj.P.Val < qval & data$logFC > posFC] <- 1
    
    # Down-regulated (Blue): adj.P.Val < qval AND logFC < negFC
    data$color_adjpval_0.05[data$adj.P.Val < qval & data$logFC < negFC] <- 2
    ...
    if (nrow(significant_DEGs) == 0) {
      ...
      # Sort by smallest adj.P.Val (most significant)
      up_data <- up_data[order(up_data$adj.P.Val), ]
      down_data <- down_data[order(down_data$adj.P.Val), ]
      ...
      # Determine horizontal line for adj.P.Val = qval
      hadjpval <- -log10(min(data$P.Value[data$adj.P.Val < qval], na.rm=TRUE))
      
      # --- Plotting ---
      p <- ggplot(data = data, aes(x = logFC, y = -log10(P.Value), color = color_adjpval_0.05)) + 
...
for (comp in comparisons) {
  ...
  for (cell in cell_types) {
    ...
    filepath <- paste0("../results/DEGs_pseudobulk_limma_voom/", cell, 
                       "/DEG_pseudobulk_voom_", cell, "_", comp_name, ".tsv")
  ...
  if (length(list_of_datasets) > 0) {
    output_file <- paste0("../results/DEGs_pseudobulk_limma_voom/", comp_name, ".xlsx")
...
output_dir <- "../results/DEGs_pseudobulk_limma_voom/"
final_volcano_dir <- "../results/volcanoes_pseudobulk/"
...
for (cell_type in cell_types) {
  ...
  for (comp in comparisons) {
    ...
    data$color_adjpval_0.05[data$adj.P.Val < qval & data$logFC > posFC] <- "Up"
    data$color_adjpval_0.05[data$adj.P.Val < qval & data$logFC < negFC] <- "Down"
    data$color_adjpval_0.05 <- factor(data$color_adjpval_0.05, 
                                     levels = c("Up", "Down", "NS"))
...
final_output_pdf <- paste0(final_volcano_dir, "disease_vs_control_combined_cell_type_volcano_plots.pdf")
...
# Set the output file path
final_output_pdf <- paste0(final_volcano_dir, "LBDtypes_vs_AD_combined_cell_type_volcano_plots.pdf")
...
# Set the output file path
final_output_pdf <- paste0(final_volcano_dir, "LBDtypes_vs_LBDtypes_combined_cell_type_volcano_plots.pdf")
