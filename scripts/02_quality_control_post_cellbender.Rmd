---
title: "10X QC processing pst cellbender"
...
order <- metadata %>%
  ...
  select(TYPE, sampleID, Sample_ID)
...
#dataObject.filtered <- readRDS(paste0("../rObjects/",treatment,"_filtered.rds"))
```
...
# Save
```{r save_object_integrated2,echo=FALSE,eval=TRUE}
...
dataObject.filtered <- readRDS(file = paste0("../rObjects/LBD_unannotated_no_integration.rds"))

```

# Azimuth
```{r Azimuth, eval=FALSE}
dataObject.filtered <- RunAzimuth(dataObject.filtered, reference = "humancortex")
...
# Save
```{r save_object_integrated1,echo=FALSE,eval=TRUE}
# Save integrated seurat object
...
#dataObject.integrated <- readRDS(paste0("../rObjects/",treatment,"_unannotated_integrated.rds"))
dataObject.integrated <- readRDS(paste0("../rObjects/cellbender_filtered.rds"))
```

# Azimuth 
https://azimuth.hubmapconsortium.org/references/#Human%20-%20Motor%20Cortex
```{r Azimuth, eval=FALSE}
dataObject.integrated <- RunAzimuth(dataObject.filtered, reference = "humancortexref")
# currently, the object has two layers in the RNA assay: counts, and data
dataObject.integrated$predicted.subclass

DimPlot(dataObject.integrated, group.by = "predicted.subclass", label = TRUE, label.size = 3) + NoLegend()
```
