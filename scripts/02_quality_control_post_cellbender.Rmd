...
i<- "BR_Nuclei_0368"
obj <- CreateSeuratObject(Read10X_h5("../cellbender/BR_Nuclei_0404/BR_Nuclei_0404_filtered_seurat.h5"))

# load Seurat (version 3) library
library(Seurat)

# load data from the filtered h5 file
data.file <- '/tgen_labs/jfryer/kolney/PMI/cellbender/B_10/B_10_cellbender_filtered_seurat.h5'
data.data <- Read10X_h5(filename = data.file, use.names = TRUE)

library(rhdf5)
h5ls(data.file)
data <- h5read(data.file, "/data")
