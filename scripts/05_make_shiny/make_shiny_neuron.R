...
# ------------------------------------------------------------------------------
dataObject <- readRDS("/tgen_labs/jfryer/kolney/LBD_CWOW/LBD_snRNA/rObjects/subclusters/CWOW_cellbender_neuron_subclustered.rds")
dataObject <- JoinLayers(dataObject)

Idents(dataObject) <- cleanObj$cell_type
dataObject@reductions$harmony
UMAP_plot <- DimPlot(dataObject, reduction = "umap.ct", label = TRUE, label.size = 3) +
  xlab("UMAP 1") +
  ylab("UMAP 2") +
  theme(
...
UMAP_plot
FeaturePlot(dataObject, features = "CUX2", reduction = "umap.ct")
help(FeaturePlot)
print(UMAP_plot)
