...
dataObject.microglia.clean <- subset(dataObject.microglia, cells = WhichCells(dataObject.microglia, idents = setdiff(unique(dataObject.microglia$seurat_clusters), c(13, 4, 5, 12))))
# DOCK8
table(dataObject.microglia$seurat_clusters)
Idents(dataObject.microglia)
VlnPlot(dataObject.microglia.clean,
        features = c("DOCK8", "P2RY12", "MBP", "RBFOX1", "APBB1IP"),
        stack = TRUE,
        flip = TRUE)

gene.names <- rownames(dataObject.microglia.clean)
microglia.genes <- c("DOCK8", "P2RY12", "APBB1IP")
neuron.genes <- c("RBFOX1", "SNAP25", "RBFOX3", "PLP1")
dataObject.microglia.clean$percent.microglia <- PercentageFeatureSet(dataObject.microglia.clean, features = microglia.genes)
dataObject.microglia.clean$percent.neuron <- PercentageFeatureSet(dataObject.microglia.clean, features = neuron.genes)

summary(dataObject.microglia.clean$percent.microglia)
summary(dataObject.microglia.clean$percent.neuron)

filtered_data <- dataObject.microglia.clean
prev_nuclei <- ncol(filtered_data)
filtered_data <- subset(filtered_data, subset = percent.microglia > 0.29 & percent.neuron == 0)
removed_count <- prev_nuclei - ncol(filtered_data)

VlnPlot(filtered_data,
        features = c("DOCK8", "P2RY12", "MBP",  "APBB1IP"),
        stack = TRUE,
        flip = TRUE)
