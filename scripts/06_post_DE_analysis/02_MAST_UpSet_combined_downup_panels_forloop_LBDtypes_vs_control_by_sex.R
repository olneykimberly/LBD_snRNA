#!/usr/bin/env Rscript
# ==============================================================================
# UpSet plots + DEG count bars for LBD types vs CONTROL, stratified by sex
#
# For each SEX (female, male) and each CELL TYPE:
#   A) DEG count bar plot (Up vs Down) for LBD_S/LBD_AS/LBD_ATS vs CONTROL
...
#
# Example:
#   DEG_microglia_LBD_S_female_vs_CONTROL_female.tsv
#   DEG_microglia_LBD_S_male_vs_CONTROL_male.tsv
#
...
output_dir      <- "/tgen_labs/jfryer/kolney/LBD_CWOW/LBD_snRNA/results/UpSet/LBDtypes_vs_control_by_sex"
...
for (sex in sexes) {
  ...
  # Comparisons for this sex (LBD only)
  ...
  plot_order <- vapply(new_plot_labels_order, function(lbl) {
    paste0(lbl, "_", sex, "_vs_CONTROL_", sex)
  }, character(1))
  ...
  for (ct in cell_types) {
    ...
    set_to_comp <- setNames(
      vapply(set_names, function(lbl) paste0(lbl, "_", sex, "_vs_CONTROL_", sex), character(1)),
      set_names
    )
  ...
  # ------------------------------------------------------------
  excel_output_path <- file.path(sex_outdir, paste0("All_CellType_DEGs_Summary_Lists_LBDtypes_vs_CONTROL_", sex, ".xlsx"))
  wb <- createWorkbook()
...
message("\nDONE. All sex-stratified LBD-type UpSet + count plots and Excel summaries are complete.")
