...
#   - Combine all comparison heatmaps (stacked vertically) into a single PDF per cell type:
#       heatmap_subcluster_ME_<ct>_comparisons.pdf
#       heatmap_subcluster_MS_<ct>_comparisons.pdf
#
# Summary table (kept):
#   summary_ME_MS_<ct>.tsv   (NO _by_comparison file)
#================================================================================
...
# Styling constants
#--------------------------------------------------------------------------------
LOLLIPOP_TITLE_SIZE <- 11

HEATMAP_TEXT_SIZE <- 9
HEATMAP_STAR_SIZE <- 2.5

# heatmap pdf sizing (slightly wider + a little shorter)
HEATMAP_BASE_WIDTH  <- 9.5
HEATMAP_BASE_HEIGHT <- 2.8     # per heatmap panel (comparison)
HEATMAP_PANEL_SPACER <- 0.15
...
make_subcluster_heatmap <- function(df, mods, title, maxval = 0.5, minval = -0.5) {
  ...
  ggplot(plot_df, aes(y = cluster, x = module, fill = avg_log2FC)) +
    geom_tile() +
    geom_text(aes(label = Significance, color = textcolor),
              size = HEATMAP_STAR_SIZE, show.legend = FALSE) +
    scale_color_identity() +
    scale_fill_gradient2(low = "#2166AC", mid = "white", high = "#B2182B") +
    Seurat::RotatedAxis() +
    theme(
      text = element_text(size = HEATMAP_TEXT_SIZE),
      plot.title = element_text(size = HEATMAP_TEXT_SIZE),
      axis.title = element_text(size = HEATMAP_TEXT_SIZE),
      axis.text = element_text(size = HEATMAP_TEXT_SIZE),
      legend.title = element_text(size = HEATMAP_TEXT_SIZE),
      legend.text = element_text(size = HEATMAP_TEXT_SIZE),
...
#----------------------------------------------------------------------------
make_interpretation <- function(dme_sig, dme_fc, dms_sig, dms_fc) {
  if (!isTRUE(dme_sig) && !isTRUE(dms_sig)) return("no_change")
  
  if (isTRUE(dme_sig) && isTRUE(dms_sig)) {
    if (!is.na(dme_fc) && !is.na(dms_fc) && sign(dme_fc) == sign(dms_fc)) {
      if (dme_fc > 0) return("ME↑ & MS↑ (coordinated activation)")
      if (dme_fc < 0) return("ME↓ & MS↓ (coordinated suppression)")
      return("ME & MS significant (direction unclear)")
    } else {
      return("ME & MS significant (discordant directions)")
    }
  }
  
  if (isTRUE(dme_sig) && !isTRUE(dms_sig)) {
    if (!is.na(dme_fc) && dme_fc > 0) return("ME↑ only (network rewiring / coordination shift)")
    if (!is.na(dme_fc) && dme_fc < 0) return("ME↓ only (network rewiring / coordination shift)")
    return("ME significant only")
  }
  
  if (!isTRUE(dme_sig) && isTRUE(dms_sig)) {
    if (!is.na(dms_fc) && dms_fc > 0) return("MS↑ only (expression change without coordination)")
    if (!is.na(dms_fc) && dms_fc < 0) return("MS↓ only (expression change without coordination)")
    return("MS significant only")
  }
  
  "no_change"
}

standardize_dme_table <- function(df) {
  out <- df
  if (!"module" %in% colnames(out) && "module_name" %in% colnames(out)) {
    out$module <- out$module_name
  }
  out |>
    dplyr::select(.data$module, .data$avg_log2FC, .data$p_val_adj) |>
    dplyr::rename(logFC = .data$avg_log2FC, FDR = .data$p_val_adj)
}
...
for (ct in cell_types) {
  ...
  has_subclusters <- "cell_type_subcluster" %in% colnames(dataObject.ct@meta.data)
  if (!has_subclusters) {
  ...
  #==============================================================================
  # A) DME (Module Eigengenes): all-cells lollipops
  #==============================================================================
  ...
  if (length(DME_list) > 0L) {
    ...
    for (cmp_name in names(DME_list)) {
      ...
      p <- as_ggplot_from_lollipop(p_raw) +
        ...
        theme(plot.title = element_text(size = LOLLIPOP_TITLE_SIZE))
  ...
  #==============================================================================
  # A2) DME subcluster loop + HEATMAPS (combined into ONE PDF stacked vertically)
  #==============================================================================
  DME_sc <- data.frame()
  ...
  if (has_subclusters) {
    ...
    if (nrow(DME_sc) > 0) {
      ...
      # build one heatmap per comparison, then stack
      heatmaps <- list()
      for (cmp in comparisons_to_read) {
        g1 <- cmp[[1]]; g2 <- cmp[[2]]
        cmp_name <- paste0(g1, "_vs_", g2)
        ...
        heatmaps[[cmp_name]] <- p_hm
      }
      
      if (length(heatmaps) > 0L) {
        p_hm_all <- patchwork::wrap_plots(heatmaps, ncol = 1)
        
        out_pdf <- file.path(out_dir, paste0("heatmap_subcluster_ME_", ct, "_comparisons.pdf"))
        pdf(out_pdf,
            width  = HEATMAP_BASE_WIDTH,
            height = HEATMAP_BASE_HEIGHT * length(heatmaps) + HEATMAP_PANEL_SPACER * (length(heatmaps) - 1))
        print(p_hm_all)
        dev.off()
        message("[", ct, "][DME] Saved combined heatmap PDF: ", out_pdf)
      }
    } else {
  ...
  #==============================================================================
  # B) DMS (Module Scores): compute scores once
  #==============================================================================
  ...
  #==============================================================================
  # B2) DMS subcluster loop + HEATMAPS (combined into ONE PDF stacked vertically)
  #==============================================================================
  DMS_sc <- data.frame()
  ...
  if (has_subclusters) {
    ...
    if (nrow(DMS_sc) > 0) {
      ...
      if (length(heatmaps) > 0L) {
        ...
        out_pdf <- file.path(out_dir, paste0("heatmap_subcluster_MS_", ct, "_comparisons.pdf"))
        pdf(out_pdf,
            width  = HEATMAP_BASE_WIDTH,
            height = HEATMAP_BASE_HEIGHT * length(heatmaps) + HEATMAP_PANEL_SPACER * (length(heatmaps) - 1))
        print(p_hm_all)
        dev.off()
        message("[", ct, "][DMS] Saved combined heatmap PDF: ", out_pdf)
      }
    } else {
  ...
  #==============================================================================
  # SUMMARY TABLE: keep ONLY summary_ME_MS_<ct>.tsv (no separate by-comparison file)
  #==============================================================================
  if (length(DME_list) > 0L && length(DMS_list) > 0L) {
    
    summary_long <- dplyr::bind_rows(lapply(names(DME_list), function(cmp) {
      dme <- if (!is.null(DME_list[[cmp]])) standardize_dme_table(DME_list[[cmp]]) else NULL
      dms <- if (!is.null(DMS_list[[cmp]])) standardize_dme_table(DMS_list[[cmp]]) else NULL
      
      merged <- dplyr::full_join(
        dme |> dplyr::rename(DME_logFC = .data$logFC, DME_FDR = .data$FDR),
        dms |> dplyr::rename(DMS_logFC = .data$logFC, DMS_FDR = .data$FDR),
        by = "module"
      )
      ...
      merged$DME_sig <- !is.na(merged$DME_FDR) & merged$DME_FDR < 0.05
      merged$DMS_sig <- !is.na(merged$DMS_FDR) & merged$DMS_FDR < 0.05
      
      merged$interpretation <- mapply(
        make_interpretation,
        dme_sig = merged$DME_sig,
        dme_fc  = merged$DME_logFC,
        dms_sig = merged$DMS_sig,
        dms_fc  = merged$DMS_logFC,
        SIMPLIFY = TRUE
      )
      ...
      merged |>
        dplyr::select(
          ...
          .data$comparison,
          .data$module,
          .data$DME_logFC, .data$DME_FDR,
          .data$DMS_logFC, .data$DMS_FDR,
          .data$interpretation
        )
    }), .id = NULL)
    
    summary_ranked <- summary_long |>
      dplyr::arrange(.data$comparison,
                     dplyr::desc(abs(.data$DME_logFC)),
                     dplyr::desc(abs(.data$DMS_logFC)))
    
    out_ranked <- file.path(out_dir, paste0("summary_ME_MS_", ct, ".tsv"))
    write.table(summary_ranked, out_ranked, sep = "\t", quote = FALSE, row.names = FALSE)
    message("[", ct, "] Saved summary table: ", out_ranked)
    
  } else {
    warning("[", ct, "] Summary not written because DME_list or DMS_list is empty.")
  }
