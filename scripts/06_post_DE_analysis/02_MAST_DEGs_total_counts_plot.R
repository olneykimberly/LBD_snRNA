...
file_df <- tibble(path = all_files) %>%
  ...
  # strip extension, split by "_" and reassemble comparison (everything after "DEG" and cell)
  mutate(noext = tools::file_path_sans_ext(bn)) %>%
  # remove leading "DEG_"
  mutate(after_deg = str_remove(noext, "^DEG_")) %>%
  # first token is cell_type
...
# --- NEW: Filter files based on the desired comparisons ---
file_df <- file_df %>% 
  filter(comparison %in% plot_order)
# --------------------------------------------------------

# read, classify, and count up/down
# -------------------------
# Replace the deg_long block
# -------------------------
...
print(final_plot)
path <- paste0("/tgen_labs/jfryer/kolney/LBD_CWOW/LBD_snRNA/manuscript_figures/DEGs_total_MAST")
print(final_plot)
...
# )
# row1
# path <- paste0("/tgen_labs/jfryer/kolney/LBD_CWOW/LBD_snRNA/manuscript_figures/Figure_2_DEGs_MAST")
# saveToPDF(paste0(path, ".pdf"), width = 7, height = 3)